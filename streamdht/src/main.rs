...
fn main() -> Result<()> {
    ...
    ssid.push_str("BIZNET").unwrap();

    let mut pass: String<64> = String::new();
    pass.push_str("12345678").unwrap();
    ...
    let mqtt_config = MqttClientConfiguration {
        ...
        username: Some("IYfqZdIZbG3Y8jCJvRAz"), // Token Device dari ThingsBoard
        password: None,
        ..Default::default()
    };
    ...
    loop {
        ...
        match Reading::read(&mut delay, &mut pin) {
            Ok(Reading { temperature, relative_humidity }) => {
                ...
                // Hitung waktu publish lokal (latensi sisi ESP)
